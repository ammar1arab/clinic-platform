...
export class AppointmentsService {
  ...
  private async resolveDiscountCode(
    ...
    if (!discountCodeId) {
      return { discount, discountType, discountReason, discountCodeId: undefined as string | undefined };
    }
  ...
  private async validateEntities(clinicId: string, dto: any) {
    const patient = await this.prisma.patient.findFirst({ where: { id: dto.patientId, clinicId, isActive: true } });
    if (!patient) throw new BadRequestException('Invalid or inactive patient');

    const doctor = await this.prisma.clinicUser.findFirst({ where: { id: dto.doctorId, clinicId, isActive: true } });
    if (!doctor) throw new BadRequestException('Invalid or inactive doctor');

    let service: any = null;
    if (dto.serviceId) {
      service = await this.prisma.service.findFirst({ where: { id: dto.serviceId, clinicId, isActive: true } });
      if (!service) throw new BadRequestException('Invalid or inactive service');
    }

    if (dto.departmentId) {
      const dept = await this.prisma.department.findFirst({ where: { id: dto.departmentId, clinicId, isActive: true } });
      if (!dept) throw new BadRequestException('Invalid or inactive department');
    }

    let room: any = null;
    if (dto.roomId) {
      room = await this.prisma.room.findFirst({ where: { id: dto.roomId, clinicId, isActive: true } });
  ...
  private validateSessionType(sessionType: string, service: any, roomId?: string, meetingUrl?: string) {
    if (service && service.supportedModes && !service.supportedModes.includes(sessionType)) {
      throw new BadRequestException(`Service does not support ${sessionType} mode`);
    }

    if (sessionType === 'in_person') {
      if (!roomId) throw new BadRequestException('Room is required for in-person appointments');
      if (meetingUrl) throw new BadRequestException('Meeting URL cannot be set for in-person appointments');
    } else if (sessionType === 'online') {
      if (roomId) throw new BadRequestException('Room cannot be set for online appointments');
      if (!meetingUrl) throw new BadRequestException('Meeting URL is required for online appointments');
      if (!meetingUrl.startsWith('https://')) throw new BadRequestException('Meeting URL must be HTTPS');
    }
  }
  ...
  private calculatePricing(
    ...
    if (discount !== undefined && discount !== null) {
      if (!discountType) throw new BadRequestException('Discount type is required when discount is provided');

      if (discount > 0 && (!discountReason || !discountReason.trim())) {
        throw new BadRequestException('Discount reason is required when a discount is applied');
      }
      ...
      throw new BadRequestException('Discount value is required when discount type is provided');
    }

    return { fee, discount, discountType, discountReason };
  }

  private async validateAvailability(clinicId: string, doctorId: string, scheduledAt: Date, durationMins: number) {
    ...
    if (hasSchedule > 0) {
      const clinicLocalStr = scheduledAt.toLocaleString('en-US', { timeZone: timezone, hour12: false });
      ...
      const endLocalStr = endAt.toLocaleString('en-US', { timeZone: timezone, hour12: false });
  ...
  private async checkConflicts(clinicId: string, doctorId: string, roomId: string | null | undefined, scheduledAt: Date, durationMins: number, excludeId?: string) {
    if (roomId) {
      const roomConflict = await this.appointmentsRepository.findConflict(clinicId, roomId, scheduledAt, durationMins, excludeId);
      if (roomConflict) throw new ConflictException('Room is already booked at this time');
    }

    const docConflict = await this.appointmentsRepository.findDoctorConflict(clinicId, doctorId, scheduledAt, durationMins, excludeId);
    if (docConflict) throw new ConflictException('Doctor already has an appointment at this time');
  }
  ...
  async create(clinicId: string, userId: string, dto: CreateAppointmentDto) {
    ...
    const durationMins = dto.durationMins ?? (service ? service.durationMins : 45);
    const scheduledAt = new Date(dto.scheduledAt);

    this.validateSessionType(sessionType, service, dto.roomId ?? undefined, dto.meetingUrl ?? undefined);
    ...
    await this.validateAvailability(clinicId, dto.doctorId, scheduledAt, durationMins);
    await this.checkConflicts(clinicId, dto.doctorId, dto.roomId, scheduledAt, durationMins);

    const { feeOverride, discountCodeId: _codeId, ...restDto } = dto;
  ...
  async findOne(clinicId: string, id: string) {
    const appointment = await this.appointmentsRepository.findById(clinicId, id);
    if (!appointment) {
  ...
  async update(clinicId: string, userId: string, id: string, dto: UpdateAppointmentDto) {
    ...
    if (dto.status && dto.status !== existing.status) {
      ...
      if (existing.status === AppointmentStatusDto.cancelled) {
    ...
    const serviceId = dto.serviceId !== undefined ? dto.serviceId : existing.serviceId;
    ...
    const meetingUrl = dto.meetingUrl !== undefined ? dto.meetingUrl : existing.meetingUrl;
    ...
    this.validateSessionType(sessionType, service, roomId ?? undefined, meetingUrl ?? undefined);

    if (dto.scheduledAt || dto.durationMins || dto.doctorId || dto.roomId) {
      await this.validateAvailability(clinicId, doctorId, scheduledAt, durationMins);
      await this.checkConflicts(clinicId, doctorId, roomId, scheduledAt, durationMins, id);
    }

    const pricing = this.calculatePricing(
      service,
      dto.feeOverride !== undefined ? dto.feeOverride : undefined,
      dto.discount !== undefined ? dto.discount : (existing.discount ? Number(existing.discount) : undefined),
      dto.discountType !== undefined ? dto.discountType : (existing.discountType ?? undefined),
      dto.discountReason !== undefined ? dto.discountReason : (existing.discountReason ?? undefined),
    );
    ...
    const appointment = await this.appointmentsRepository.update(clinicId, id, updateData);
    this.dashboardGateway.emitAppointmentChanged(clinicId);
    return appointment;
  }

  async markPaid(clinicId: string, clinicUserId: string, id: string, dto: MarkPaidDto) {
    ...
    const appointment = await this.appointmentsRepository.markPaid(clinicId, id, {
      isPaid: true,
      paidAt: new Date(),
  ...
  async markUnpaid(clinicId: string, id: string) {
    ...
    const appointment = await this.appointmentsRepository.markUnpaid(clinicId, id);
    this.dashboardGateway.emitAppointmentChanged(clinicId);
    return appointment;
  }
}