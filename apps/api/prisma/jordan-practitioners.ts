...
const HOSPITALS = [
  ...
  { en: "the Islamic Hospital", ar: "المستشفى الإسلامي" },
  {
    ...
    ar: "مستشفى الملك المؤسس عبد الله الجامعي",
  },
...
const ROSTER: RosterRow[] = [
  ...
  },
  {
    ...
    title: "Consultant",
    nat: "JO",
    spec: "General Pediatrics",
    specAr: "طب الأطفال العام",
    dept: "Pediatrics",
    langs: ["ar", "en", "fr"],
    uni: 0,
    hosp: 1,
  ...
  },
  {
    ...
    title: "Therapist",
    nat: "JO",
    spec: "Musculoskeletal Physiotherapy",
    specAr: "علاج طبيعي للجهاز الحركي",
    dept: "Physiotherapy",
    langs: ["ar", "en"],
    uni: 3,
    hosp: 1,
  ...
  },
  {
    ...
    title: "Specialist",
    nat: "LB",
    spec: "Cosmetic Dermatology",
    specAr: "الجلدية التجميلية",
    dept: "Dermatology",
    langs: ["ar", "en", "fr"],
    uni: 8,
    hosp: 4,
...
function timeOffsFor(index: number): SeedPractitioner["timeOffs"] {
  if (index % 7 === 0) {
    return [
      { startOffset: 18, endOffset: 22, reason: "Eid al-Fitr family leave" },
    ];
  }
  if (index % 7 === 2) {
    return [
      {
        startOffset: 40,
        endOffset: 42,
        reason: "Jordan Medical Association conference",
      },
    ];
  }
  if (index % 7 === 4) {
    ...
    return [
      {
        startOffset: 28,
        endOffset: 29,
        reason: "Royal Medical Services CME day",
      },
    ];
  }
  return [];
...
export function buildJordanianPractitioners(): SeedPractitioner[] {
  if (ROSTER.length !== 50) {
    throw new Error(
      `Expected 50 Jordanian practitioners, got ${ROSTER.length}`,
    );
  }
  ...
  const built = ROSTER.map((row, index) => {
    ...
    const whatsapp =
      index % 8 === 3
        ? jordanMobile(index + 80, prefix === "79" ? "78" : "79")
        : phone;
    ...
    const experienceYears = Math.max(
      3,
      Math.min(40, 2026 - (row.y + graduateAge)),
    );
    ...
    const licenseExpiry = new Date(
      Date.UTC(2027 + (index % 3), index % 12, 1 + (index % 27)),
    );
