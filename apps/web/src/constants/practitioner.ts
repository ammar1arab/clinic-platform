...
export function applyPractitionerDirectory(
  ...
  const filtered = items.filter((p) => {
    ...
    if (filters.nationality && p.nationality !== filters.nationality)
      return false;
    if (filters.license !== 'all' && licenseBucket(p, now) !== filters.license)
      return false;
    if (filters.experience !== 'all') {
      if (p.experienceYears == null) return false;
      if (experienceBucket(p.experienceYears) !== filters.experience)
        return false;
    }
    if (term) {
      const fields = [
